...
* content
{:toc}
I want to compare 5 things: `char ** a;` `const char ** b;` `char * const * c;` `const char * const * d; const char * const * const e;`
...
There's actually a very simple way to read: read the definition from right to left, the result is - `char** s;` `s` is a `pointer 1`, which points to a `pointer 2`, which points to a `pointer 2char`
